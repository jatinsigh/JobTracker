...
public interface IUserInterface extends JpaRepository<User, String>{
	  ...
	  @Query(value="SELECT jobId, jobRole, postedOn, companyName from "
		  		+ " jobPost a NATURAL JOIN jobSkills b JOIN userSkills c WHERE c.userskill = a.jobskill WHERE c.userid=?", nativeQuery=true)
